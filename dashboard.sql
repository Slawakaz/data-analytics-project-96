...
lead_times AS (
    SELECT
        EXTRACT(EPOCH FROM (l.created_at - fv.first_visit)) / 86400 
            AS days_to_close
    FROM leads AS l
    JOIN first_visits AS fv 
        ON l.visitor_id = fv.visitor_id
...
SELECT
    ...
    ROUND(
        COALESCE(ac.total_cost / NULLIF(cd.visitors, 0), 0), 
        2
    ) AS cpu,
    ROUND(
        COALESCE(ac.total_cost / NULLIF(cd.leads, 0), 0), 
        2
    ) AS cpl,
    ROUND(
        COALESCE(ac.total_cost / NULLIF(cd.purchases, 0), 0), 
        2
    ) AS cppu,
    ROUND(
        COALESCE(
